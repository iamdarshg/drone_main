...
static void update_sensor_readings(sensor_state_t *sensor) {
    ...
    sensor->state.acc[0] = sensor->state.acc[0] + G * sin_pitch;
    sensor->state.acc[1] = sensor->state.acc[1] - G * sin_roll * cos_pitch;
    sensor->state.acc[2] = sensor->state.acc[2] - G * cos_roll * cos_pitch;
    
    // Update gyroscope readings if applicable (LSM6DS3)
    if (sensor == &sensors[SENSOR_LSM6DS3]) {
        sensor->state.gyro[0] = sensor->state.gyro[0];
        sensor->state.gyro[1] = sensor->state.gyro[1];
        sensor->state.gyro[2] = sensor->state.gyro[2];
    }
...
const char *host_peripherals_status(void) {
    ...
    // Count powered rails
    int powered = 0;
    for (int i = 0; i < sizeof(power_state); i++) {
        powered += power_state[i];
    }
    
    n += snprintf(buf + n, sizeof(buf) - n, "Power rails: %d enabled\n", powered);
